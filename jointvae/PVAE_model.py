...
class VAE(nn.Module):
    def __init__(self, img_size, latent_spec, droprate_init=0.2, weight_decay=0.001, lambda_l0=0.1, temperature=.1,
                 use_cuda=True):
        """
        ...
        # Add final layers
        encoder_layers += [
            nn.Conv2d(32, 64, (2, 2), stride=2, padding=0),
            #            nn.Conv2d(32, 32, (4, 4), stride=2, padding=1),
            nn.ReLU(),
            nn.Conv2d(64, 64, (2, 2), stride=2, padding=0),
            nn.ReLU()
        ]
        ...
        if self.is_continuous:
            ...
            self.fc_latent = nn.Sequential(
                L0Pair(self.hidden_dim, self.latent_cont_dim, droprate_init=self.droprate_init,
    ...
    def encode(self, x):
        ...
        #            regularization += self.lambda_l0*self.fc_log_var[0].regularization().cuda()/162079.
        ...
        return latent_dist, mask, regularization
    ...
    def sample_normal(self, mean, var):
        """
    ...
    def forward(self, x):
        ...
        x : torch.Tensor
            Batch of data. Shape (N, C, H, W)
        """
        latent_dict, mask, regularization = self.encode(x)
        latent_sample = self.reparameterize(latent_dict).cuda()
        # latent_sample,regularization = self.pruning(latent_sample)
        return torch.sigmoid(self.decode(latent_sample)), latent_dict, mask, regularization