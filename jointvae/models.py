...
#         return self.pruning_latent(latent_sample), regularization
    ...
    def forward(self, x):
        ...
        x : torch.Tensor
            Batch of data. Shape (N, C, H, W)
        """
        latent_dist,mask, regularization = self.encode(x)
        latent_sample = self.reparameterize(latent_dist).cuda()
        #latent_sample,regularization = self.pruning(latent_sample)
        return self.decode(latent_sample), latent_dist, mask, regularization
