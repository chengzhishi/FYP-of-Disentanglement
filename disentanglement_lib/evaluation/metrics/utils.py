...
def generate_batch_factor_code(ground_truth_data, representation_function,
  ...
  while i < num_points:
    ...
    if i == 0:
      ...
      representations = representation_function(current_observations).detach().cpu()
    else:
      factors = np.vstack((factors, current_factors))
      representations = np.vstack((representations,
                                   representation_function(
                                       current_observations).detach().cpu()))
    i += num_points_iter
  return np.transpose(representations), np.transpose(factors)[1:,:]
...
#                      discretizer_fn=@histogram_discretizer):
  """Wrapper that creates discretizers."""
  return histogram_discretizer(target, num_bins)
